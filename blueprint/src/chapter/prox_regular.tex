...
$$
<<<<<<< codex/create-latex-proof-for-proposition-3.2-eruylr
\|\text{proj}_S(u_1)-\text{proj}_S(u_2)\|\le (1-\gamma)^{-1}\|u_1-u_2\|.
=======
\|\proj_S(u_1)-\proj_S(u_2)\|\le (1-\gamma)^{-1}\|u_1-u_2\|.
>>>>>>> master
$$
Hence (2) holds.
...
=======
so $x+tv\in U_\rho^\gamma(S)$ and $\proj_S(x+tv)$ is well-defined by (2). Using the projection/resolvent identification for the proximal normal cone from Theorem~3 (constant-radius case), one gets $\proj_S(x+tv)=x$. Therefore $x\in\Proj_S(x+tv)$, i.e. $S$ is $\rho$-uniformly prox-regular.
>>>>>>> master
...
=======
Fix $\gamma\in(0,1)$ and $x\in U_\rho^\gamma(S)$, and write $p=\proj_S(x)$. By (2), $\proj_S$ is Lipschitz on the tube, which yields $C^{1,1}$ regularity of $x\mapsto \frac12 d_S(x)^2$ there, with modulus controlled by $(1-\gamma)^{-1}$. Translating this estimate to $d_S$ via proximal subgradients gives
>>>>>>> master
$$
