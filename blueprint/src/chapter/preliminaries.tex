...
\uses{def:distance}
\lean{MoreauSweeping.proximalNormalCone}
\leanok
Let $S \subset H$ be a closed set and $x \in S$. A vector $\zeta$ belongs to the proximal normal cone to $S$ at $x$ if there exist $\sigma \ge 0$ and $\eta > 0$ such that
$$\langle \zeta, y - x \rangle \le \sigma \|y-x\|^2$$
for all $y \in S$ with $\|y-x\| < \eta$.
\end{definition}

\begin{lemma}[Projection formula for the proximal subdifferential of the distance]
\label{lem:proximal_normal_characterization}
\uses{def:distance,def:proximal_subdifferential}
...
\uses{def:approximate_projection,lem:proximal_normal_characterization}
